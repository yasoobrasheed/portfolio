...
export const Insite = () => {
  ...
  const htext2 = "It all begins with a circle... or rather a 100-gon"
  ...
  const htext3 = "Using TestFlight, we soft launch Insite on our friends' phones to test our geofences"
  const text3 = "Our iOS application is just a simple usage of Apple's Map API and Core Location API. Core Location has a geofence feature, so we feed the geofences we generated using Python into the iOS app. On the bottom right, you can see some of the sample push notifications we sent to our test users when they walked in and out of geofences on campus. We also include the accuracy of a person's location to determine just how accurate Apple's GPS technology is when it comes to geofence interation. GPS is only accurate within a 5-10m margin."
  const htext4 = "We bring Insite to a close with a couple extra features, our backend server, and a short privacy survey for our users."
